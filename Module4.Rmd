...
tree_rf
```
We observe an out-of-bag estimated error rate of about 4.5%, which translate to an accuracy of about 95.5%. This is better than the observed accuracy from the decision tree model. Additionally the class error rate for the clean emails is even lower than the overall estimated error, resulting in a low rate of incorrectly labelling clean emails as spam.
...
We now check that k = 5 is the value of k that gives the best accuracy by using repeated cross-validation, with an out-of-sample accuracy of 80.92%.
